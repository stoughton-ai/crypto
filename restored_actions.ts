...
Fear & Greed: ${fng}/100 (${marketStats?.fearGreedStatus || 'Unknown'})`;
        ...
        if (holding && holding.amount > 0) {
              ...
              if (resolvedZone !== 'CONVICTION' && newCycles >= gpmConfirmNeeded && !alreadyScaledDownInThisZone) {
                ...
                if (isArrivalScaleUp && scaleUpCooldownMet) {
                  ...
                  if (topUpUsd >= 15 && !arena.sellOnlyMode) {
                    ...
                    crossPoolBuys.push({ pool, ticker: upper, smoothedScore, price: priceData.price, reflection: scaleUpReflection, marketContext, isGpmScaleUp: true });
          ...
          if (smoothedScore >= effectiveBuyThreshold) {
            ...
            if (!arena.sellOnlyMode) {
                ...
                crossPoolBuys.push({ pool, ticker: upper, smoothedScore, price: priceData.price, reflection: buyReflection, marketContext });
  ...
  if (crossPoolBuys.length > 0 && !arena.sellOnlyMode) {
    ...
    for (const signal of crossPoolBuys) {
      ...
      // For GPM scale-ups: top-up to maxAlloc
      // For rebounds and new entries: standard maxAlloc sizing
      // For buybacks: use the stage amount
      const holdingValue = holding ? holding.amount * signal.price : 0;
      let targetBuyUsd = Math.min(availableSharedCash, maxAlloc - holdingValue);
      ...
      if (revolutResult.success) {
        ...
        const result = await executePoolBuy(userId, pool, signal.ticker, buyAmount, actualPrice, signal.reflection, signal.marketContext, signal.reflection);
        if (result.success && result.trade) {
          pool.cashBalance = 0;
...
async function executeRevolutTrade(
  ...
  try {
    // HALT ALL REAL TRADING (Emergency override per user request)
    console.warn(`[Revolut] Emergency halt: Real trading is disabled. Blocked ${side} for ${ticker}.`);
...
export async function getArenaStatus(userId: string, assetClass: AssetClass = 'CRYPTO'): Promise<{
  ...
  const [arena, trades, marketStats, eodhd, tokenAnalyses] = await Promise.all([
  ...
  return serialize({ arena, trades, marketStats, eodhd, tokenAnalyses });
}
...
export async function syncMasterPortfolioFromRevolut(userId: string, arena: ArenaConfig) {
  ...
  const configDoc = await adminDb.collection('agent_configs').doc(userId).get();
  const agentConfig = configDoc.data();
  if (!agentConfig?.revolutApiKey || !agentConfig?.revolutPrivateKey) return;
...
export async function getGranularPriceMovements(tickers: string[]) {
  ...
  const intervals = [5, 15, 30, 60, 180, 360, 720, 1140];
...
export async function getUnifiedAuditTrail(userId: string) {
  ...
  return serialize(events.slice(0, 100));
}
